...
class CourantArtikelTest {
    ...
    @ParameterizedTest
    @CsvSource({
            "90.00, true",     // Below minimum price (purchase price + margin)
            "110.00, false",   // Valid discount price below normal price
            "119.99, false",   // Just below normal price
            "120.00, true",    // Equal to normal price
            "121.00, true"     // Above normal price
    })
    void startKorting_ValidatieKortingsprijs(String prijsInKorting, boolean verwachtException) {
        BigDecimal testPrijsInKorting = new BigDecimal(prijsInKorting);

        if (verwachtException) {
            assertThrows(IllegalArgumentException.class, () ->
                    artikel.startKorting(LocalDate.now().plusDays(1), 5, testPrijsInKorting));
        } else {
            assertDoesNotThrow(() ->
                                       artikel.startKorting(LocalDate.now().plusDays(1), 5, testPrijsInKorting));
        }
