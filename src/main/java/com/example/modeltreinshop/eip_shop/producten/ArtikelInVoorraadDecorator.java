...
public class ArtikelInVoorraad extends ArtikelDecorator {
    ...
    public BigDecimal getWinstmarge() {
        Artikel artikel = super.getGedecoreerdArtikel();
        if (!(artikel instanceof ArtikelInVoorraad voorraadArtikel)) {
            throw new IllegalArgumentException("Korting kan alleen toegepast worden op ArtikelInVoorraad");
        }
        BigDecimal aankoopprijs = voorraadArtikel.getLaatsteAankoopprijs();
        BigDecimal winstmarge = voorraadArtikel.getMinimaleWinstmarge();
        WinstmargeType margeType = voorraadArtikel.getWinstmargeType();

        return switch (margeType) {
            case EURO -> aankoopprijs.add(winstmarge);
            case PERCENTAGE -> aankoopprijs.multiply(
                    BigDecimal.ONE.add(winstmarge.divide(BigDecimal.valueOf(100)))
            );
        };
    }
}
